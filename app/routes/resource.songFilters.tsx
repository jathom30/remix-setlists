...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const tempoError =
    !Array.isArray(tempos) ||
    [...tempos].some((tempo) => typeof tempo !== "string");
  const feelsError =
    !Array.isArray(feels) ||
    [...feels].some((feel) => typeof feel !== "string");
  const isCoverError = typeof isCover !== "string";
  const positionsError =
    !Array.isArray(positions) ||
    [...positions].some((position) => typeof position !== "string");
  const formHasError =
    tempoError || feelsError || isCoverError || positionsError;
