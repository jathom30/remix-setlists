...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const { fields, errors } = getFields<{ role: "ADMIN" | "MEMBER" | "SUB" }>(
    formData,
      ...
      <FlexList gap={2}>
        ...
        <Tabs
          tabs={[
            {
              label: "Admin",
              isActive: roleTab === RoleEnum.ADMIN,
              onClick: () => setRoleTab(RoleEnum.ADMIN),
            },
            {
              label: "Member",
              isActive: roleTab === RoleEnum.MEMBER,
              onClick: () => setRoleTab(RoleEnum.MEMBER),
            },
            {
              label: "Sub",
              isActive: roleTab === RoleEnum.SUB,
              onClick: () => setRoleTab(RoleEnum.SUB),
            },
          ]}
        >
          <FlexList>
            <p className="text-sm">
              {roleExplanitoryText[roleTab || "NOT_FOUND"]}
            </p>
            <Form method="put">
              <FlexList>
                {!canRemoveAsAdmin && roleTab !== RoleEnum.ADMIN ? (
                  <p className="text-sm text-error">
                    You are the only <strong>admin</strong> on this band. Make
                    at least one other member an Admin before updating your
                    role.
                  </p>
                ) : null}
                <Button
                  type="submit"
                  isDisabled={roleTab === member.role || !canRemoveAsAdmin}
                  name="role"
                  value={roleTab as unknown as string}
                  kind="secondary"
                >
                  {roleTab === member.role
                    ? "Current role"
...
const roleExplanitoryText: Record<string, string> = {
  ADMIN:
    "Admins can read, create, update, and delete songs and setlists. They can also add, remove, promote, and demote members. Additionally, they are able to update the band name and icon. Lastly, they are able to delete the band.",
