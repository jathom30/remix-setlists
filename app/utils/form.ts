...
export const getFields = <T>(
  formData: FormData,
  schema: { name: keyof T; type: string; isRequired: boolean }[],
) => {
  ...
  const fieldErrors = schema.reduce<Record<keyof T, string | undefined>>(
    (acc, field) => {
      ...
      const hasError: boolean =
        isRequired &&
        (value === null || typeof valueAsType(type, value) !== type);
      if (hasError) {
        return {
          ...acc,
